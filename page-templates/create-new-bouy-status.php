...
if ( 'Submit' === $_POST['submit'] ) {
	...
	$post_url     = 'https://saveontarioshipwrecks.ca/buoysites/' . get_the_title( $_POST['site_name'] );
	...
	$querylatlong = "SELECT C.lid AS lid, D.latitude, D.longitude
	...
	$post_name_formatted = str_replace(
		array( 'https://saveontarioshipwrecks.ca/buoysites/', '-' ),
		array( '', ' ' ),
		$site_id
	);
...
if ( $current_user->has_cap('publish_buoy_site') || $current_user->has_cap('publish_buoy_status')  || $current_user->has_cap('activate_plugins') ) {
