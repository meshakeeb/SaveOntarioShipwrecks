...
function spam_cleanup() {
    ...
    foreach($members as $member){
        $subscription =  $wpdb->get_row("SELECT * FROM `sos807_pms_member_subscriptions` WHERE `user_id`= $member->ID", OBJECT);
      
  
        if( $subscription->status === "pending" && ( strtotime($subscription->start_date) < strtotime( '-5 days' ) ) ){
            $output .= '<li>';        
