...
class Theme_Setup {
    ...
    public function __construct() {
        ...
        // $this->filter( 'gettext', 'custom_text_strings', 20);
     ...
     * Custom text string.
     *
     * @param  string $translated_text Text to be translated.
     * @return string
     */
    public function custom_text_strings( $translated_text ) {
        $hash = [
            'Abandon' => __( 'Delete Permanently', 'ontario' ),
            'Upgrade' => __( 'Change', 'ontario' ),
        ];

        return isset( $hash[ $translated_text ] ) ? $hash[ $translated_text ] : $translated_text;
    }
    ...
    public function add_loginout_link( $items, $args ) {
        if ( ! is_user_logged_in() || ! in_array( $args->theme_location, [ 'header_menu' ] ) ) {
            return $items;
        }
    ...
    public function prefix_load_cat_posts () {
        ...
        $the_query = 'all' == $cat_id ? new WP_Query( [ 'posts_per_page' => '10' ] ) :
            new WP_Query( [ 'posts_per_page' => '5', 'cat' => $cat_id ]);
