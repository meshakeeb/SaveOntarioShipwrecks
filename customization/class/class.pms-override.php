...
class BoltPMS
{
              ...
              $html .='<div class="col-sm-6">';
                $html .='<div class="custom-select">';
                  ...
                  $html .='<option value="">Chapter *</option>';
                    foreach( BoltMedia::chapter_list() as $chapter ){
                      $html .= '<option value="'.$chapter->ID.'">'.$chapter->post_title.'</option>';
                    }
                  $html .='</select>';
                $html .='</div>';  
    ...
    function pmsAboutToExpire()
    {
        global $wpdb;
        $table_subscription = $wpdb->prefix."pms_member_subscriptions";
        $table_user = $wpdb->prefix."users";
       
        $users = $wpdb->get_results(
            "SELECT * FROM 
            $table_subscription as SUBSCRIPTION
    ...
    function pmsReminder()
    {
        $users = $this->pmsAboutToExpire();
        ...
        foreach ($users as $user) {
            $mail = get_option('email_reminder');

            $date_expire = $user->expiration_date;    
            $date = new DateTime($date_expire);
            $now = new DateTime();

            $search = array('{fname}', '{lname}', '{days}', '{expiration}', '{account_type}', '{login_link}');
            $replace = array(
            ...
            $mail_content = str_replace($search, $replace, $mail['content']);
            
            if (isset($_GET['debug']) && $_GET['debug'] === "on") {
                echo wpautop($mail_content).'<hr>';    
            } else {
                $mail = @wp_mail(
                    $user->user_email, 
                    $mail['title'], 
                    wpautop($mail_content), 
                    $headers
                );                
            }
        } 
