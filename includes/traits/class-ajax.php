 ...
 * @since      1.0.0
 * @package    Ontario
 * @subpackage Ontario\Traits
 * @author     BoltMedia <info@boltmedia.ca>
 */

namespace Ontario\Traits;
...
trait Ajax {
	...
	protected function ajax( $tag, $function_to_add, $priority = 10 ) {
		\add_action( 'wp_ajax_ontario_' . $tag, [ $this, $function_to_add ], $priority );
	}
	...
	public function verify_nonce( $action ) {
		if ( ! isset( $_REQUEST['security'] ) || ! \wp_verify_nonce( $_REQUEST['security'], $action ) ) {
			$this->error( __( 'Error: Nonce verification failed', 'ontario' ) );
		}
	}
