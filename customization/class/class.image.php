...
class BoltImage {
	...
	public static function PDFBadge ($bolt_user){

		//header("Content-type: image/png");		
		$im    = imagecreatefrompng( get_theme_file_path().'/customization/assets/images/pdf-card.png');
		$color = imagecolorallocate($im, 0, 0, 0);

		//$font = imageloadfont('fonts/arial.gdf');
		$font = get_theme_file_path().'/customization/assets/fonts/arial.ttf';
		$font_bold = get_theme_file_path().'/customization/assets/fonts/arial-bold.ttf';
		$font_italic = get_theme_file_path().'/customization/assets/fonts/arial-italic.ttf';
		$font_bold_italic = get_theme_file_path().'/customization/assets/fonts/arial-bold-italic.ttf';

		$font_size1 = 16;
		$string1 = $bolt_user['fname'].' '.$bolt_user['lname'];
		...
		$font_size7 = 8;
		$string7 = 'expires '.$bolt_user['expiry'];
		...
		$rendered_image = ( imagepng($im, wp_upload_dir()['basedir'].'/member_badges/user_'.$bolt_user['ID'].'.png' ) );
		
		if($rendered_image){
			imagedestroy($im);		
			return wp_upload_dir()['basedir'].'/member_badges/user_'.$bolt_user['ID'].'.png';			
		}
